...
function formatEODLine(outcomeName: string, formulaId: number, data: CountedData, isTeam: boolean): string | null {
  ...
  switch (formulaId) {
    ...
    case 9: {                                                     // Job Details
      ...
      const lines = jobDetails.map(j => `${j.contactName} ${formatDollar(j.value)} ${displayLabel(j.source) || "N/A"} - ${cleanAddress(j.address) || "N/A"}`);
      ...
      if (isYear) {
        ...
        if (jobDetails.length > 0) {
          for (const j of jobDetails) {
            lines.push(`${j.contactName} ${formatDollar(j.value)} ${displayLabel(j.source) || "N/A"} - ${cleanAddress(j.address) || "N/A"}`);
          }
